import { getTwitchFollowedStreams, getTwitchStreams, getTwitchUsers, validateTwitchToken } from "./twitchRepository";
...
import { FollowedStream, FollowedStreamResponse, TopStream, TopStreamResponse } from "../../domain/twitch/service";
import { createFollowedTopStream } from "./twitchHelpers";
import { ONE_MINUTE_IN_MILLISECONDS, POOLING_JUST_WENT_LIVE } from "../../domain/background/constants";
 ...
 */
export const getCurrentUser = async (): Promise<ValidateTokenResponse> => validateTwitchToken();
...
export const getFollowedStreams = async (
	...
	const streams: GetFollowedStreamsResponse = await getTwitchFollowedStreams(userId, cursor, first);

	const promisesGetExtraInfo = streams.data.map(async (stream: TwitchFollowedStream): Promise<FollowedStream> => {
	...
	const responseLivestreams: PromiseSettledResult<FollowedStream>[] = await Promise.allSettled(promisesGetExtraInfo);

	responseLivestreams.forEach((result: PromiseSettledResult<FollowedStream>) => {
...
export const getAllFollowedStreams = async (userId: string): Promise<FollowedStream[]> => {
	try {
...
export const getJustWentLive = async () => {
	...
	const livestreams: FollowedStream[] = await getAllFollowedStreams(user.user_id);

	const nowUTC: Date = new Date(new Date().getTime());

	return livestreams.filter(
		(stream: FollowedStream) =>
			Math.round((nowUTC.getTime() - new Date(stream.started_at).getTime()) / ONE_MINUTE_IN_MILLISECONDS) <=
			POOLING_JUST_WENT_LIVE,
	);
};

export const getTopStreams = async (cursor?: string): Promise<TopStreamResponse> => {
	const streams: GetStreamsResponse = await getTwitchStreams(cursor);

	const promisesGetExtraInfo = streams.data.map(async (stream: TwitchStream): Promise<TopStream> => {
		const streamer: TwitchUser = await getUserById(stream.user_id);
	...
	const responseLivestreams: PromiseSettledResult<TopStream>[] = await Promise.allSettled(promisesGetExtraInfo);
