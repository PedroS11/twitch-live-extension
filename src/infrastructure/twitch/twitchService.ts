...
import {getGames, getStreams, getUserFollows, getUsers, MAX_INTEGER_VALUE} from "./twitchRepository";
...
export const getFollowedLivestreams = async (userIds: string[]): Promise<FollowedLivestream[]> => {
    ...
    const promisesGetExtraInfo = streams.map(async(stream: GetStream): Promise<FollowedLivestream> => {
    ...
    const responseLivestrams: PromiseSettledResult<FollowedLivestream>[] = await Promise.allSettled(promisesGetExtraInfo);
