...
export const getNotificationFlagFromStorage = async (): Promise<boolean> =>
	(await chrome.storage.sync.get(NOTIFICATIONS_ENABLE_KEY))[
		NOTIFICATIONS_ENABLE_KEY
	] === "true";
...
export const getTokenFromStorage = async (): Promise<string> =>
	(await chrome.storage.sync.get(TOKEN_KEY))[TOKEN_KEY] || "";
...
export const storeNotificationsFlagOnStorage = async (state: boolean) =>
	await chrome.storage.sync.set({ [NOTIFICATIONS_ENABLE_KEY]: state + "" });
